...
DOMAIN="2.aaa-jp01.yunti.best"                            # 子域名
...
# 运行第二个容器
docker run --restart=on-failure --name a.ssrjp-02-04 -d \
-v /etc/soga/:/etc/soga/ --network host \
-e type=sspanel-uim \
-e server_type=ssr \
...
# 运行第三个容器
docker run --restart=on-failure --name a.jp-01-03 -d \
-v /etc/soga/:/etc/soga/ --network host \
-e type=sspanel-uim \
-e server_type=v2ray \
...
chmod +x nezha-agent && \
./nezha-agent service install -s 15.235.144.68:5555 -p FJgApJYmdwgkY8f3Jr
...
port:22,24,25,26,50,57,105,106,109,110,143,158,209,465,587,995,1109,7081
EOF
